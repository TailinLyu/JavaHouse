...
const Drinks = sequelize.define('User', {
...
app.get('/drinks', async (req,res) => {
    const drinks = await Drinks.findAll()//.then((data) => res.send(data));   
    res.send(drinks) 
})
...
app.patch('/drinks/:id', async (req, res) => {
    ...
    drink.recipe = recipe
    drink.save().then(() => console.log(`Drink has been updated by ${drinkName} with ${recipe}`))
    res.send(`Drink has been updated by ${drinkName} with ${recipe}`)
})

app.delete('/drinks/:id', (req, res) =>{
    const drink = Drinks.build({drinkId: req.params.id})
    drink.destroy().then(() => res.send(`The Drink ${req.params.id} has been deleteed`))
})
